package otus.project.feature.map
...
class MapAdapter(val mapView: MapView) {
    private var clickMarker : (id : Long, place : Pair<Float, Float>) -> Unit = { _, _ -> }
    private var clickEmpty : (place : Pair<Float, Float>) -> Unit = { _ -> }

    private fun onClickMarker(ids : String, loc : MapLocation) {
        // convert parameters and call clickMarker()
        val id : Long = ids.toLong()
        val place : Pair<Float, Float> = Pair(loc.latitude?.toFloat() ?: 0f, loc.longitude?.toFloat() ?: 0f)
    ...
    fun setListeners(onMarker : (id : Long, place : Pair<Float, Float>) -> Unit, onEmpty : (place : Pair<Float, Float>) -> Unit) {
    ...
    fun locate(place : Pair<Float, Float>) {
        val loc = LatLon(place.first.toDouble(), place.second.toDouble())
        mapView.getMapAsync { map -> map.flyTo(loc, true) }
    }

    fun addMark(id : Long, place : Pair<Float, Float>) {
        val ids = id.toString()
        val loc = MapLocation(place.first.toDouble(), place.second.toDouble())
